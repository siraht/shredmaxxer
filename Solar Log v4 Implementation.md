...
Default remains frictionless and local-only. Optional hardening (off-by-default):
...
**Local-first** with **IndexedDB primary** + fallback:

* **IndexedDB** (primary): async, higher quota, better for per-day records, transactional
* fallback: localStorage (legacy/compat only)
* Export/import stays the portability layer
...
interface SegmentLog {
  ...
  tsFirst?: string;      // set on first user touch
  tsLast?: string;       // set on edits
  rev: number;           // monotonic per-segment revision for merge/conflict resolution
}
...
interface Meta {
  ...
  installId: string;     // stable per-install identifier
